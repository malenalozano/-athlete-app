...
export function BibliotecaCientifica() {
  // TODO: fetch studies from database / Supabase
  const studies: {
    id: number;
    title: string;
    category: string;
    ...
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold text-white flex items-center gap-2 mb-2">
          <BookOpen className="h-5 w-5 text-[#C9FF00]" />
      ...
      <Card className="bg-[#161B22] border border-[#C9FF00]/30 rounded-xl mb-8">
        ...
        <CardContent>
          <form className="space-y-6">
            ...
            <div>
              <Label className="text-white mb-2 block">Resumen Manual (opcional)</Label>
              <Textarea
        ...
        {studies.length === 0 ? (
          ...
          <div className="space-y-4">
            {studies.map((study) => (
              <Card
                key={study.id}
                className="bg-[#161B22] border border-[#C9FF00]/30 rounded-xl hover:border-[#C9FF00]/60 transition-colors"
              >
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <CardTitle className="text-white text-base mb-2">{study.title}</CardTitle>
                ...
                <CardContent>
                  <p className="text-sm text-[#8B949E] mb-4">{study.summary}</p>
